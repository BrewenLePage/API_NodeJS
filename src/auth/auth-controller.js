...
const JWT_SECRET = process.env.PORT || jsonConfig.jwtSecret;
...
// #swagger.tags = ['Auth']
// #swagger.description = 'Service to register new user into cars'
// #swagger.summary = 'Service to register new user'
    ...
    bcrypt.compare(password, userData.password, function(err, result) {
        if(result == false){
            ...
            let jwtToken = createJwtToken(userData.uid, userData.email, userData.firstName, userData.lastName);
...
// #swagger.tags = ['Auth']
// #swagger.description = 'Service to get authenticated user infomation'
// #swggaer.summary = 'Service to get authenticated user'
/* #swagger.security = [{
            "bearerAuth": []
     }] */
    ...
    let userSnapshot = await firestore.collection("users").where("email", "==", decoded.username);
    let userData = (await userSnapshot.get()).docs[0];
    userData = userData.data();
    userData.password = "";
